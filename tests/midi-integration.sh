...
result=$(eval timeout 15 $CLI --address $BRIDGE upload $TEST_CONFIG 2>&1)
...
failures=0
for i in 0 1; do
  result=$(eval timeout 5 $CLI --address $BRIDGE read $i 2>&1)
...
result=$(eval timeout 5 $CLI --address $BRIDGE read 1 2>&1)
...
EOF
result=$(eval timeout 15 $CLI --address $BRIDGE upload $GLOBAL_TEST 2>&1)
...
presets:
  - name: "Solo Preset"
    buttons:
      A: { label: "X", cc: 1, color: red }
EOF
eval timeout 15 $CLI --address $BRIDGE upload $SINGLE_TEST 2>&1 > /dev/null
rm -f "$SINGLE_TEST"
sleep 1
...
result=$(eval timeout 5 $CLI --address $BRIDGE status 2>&1)
if [[ "$result" == *"Presets:"* ]] && [[ "$result" == *"Flash fmt:"* ]]; then
  loaded=$(echo "$result" | grep "Presets:" | awk '{print $2}')
  if [[ "$loaded" == "1" ]]; then
...
sleep 1
result=$(eval timeout 5 $CLI --address $BRIDGE read 0 2>&1)
if [[ "$result" == *"Compiler Test"* ]] && [[ "$result" == *"Clean"* ]] && [[ "$result" == *"Crunch"* ]] && [[ "$result" == *"Lead"* ]]; then
  echo "✓ (compiled preset with 3 generated snapshot buttons)"
else
  echo "✗ (unexpected content)"
  echo "$result"
  rm -f "$COMPILED_BIN"
